...
To compile the editor, you will need many of the same tools as to compile Endless Sky itself. Go see its build instructions for more details. Qt is also required, but vcpkg can acquire this if you use that. Binary releases will happen occasionally, but are not as high a priority right now as the development of the game itself.
