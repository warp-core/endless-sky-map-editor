...
# Builds Endless Sky Editor and packages it as AppImage
...
mv AppDir/usr/share/games/endless-sky-map-editor/* AppDir/
...
OUTPUT=es-temp.AppImage ./linuxdeploy --appdir AppDir -e "$1/endless-sky-map-editor" --output appimage
...
if [ ! -z "$OUTPUT" ] ; then
  ...
  mv ./Endless_Sky_Editor-000-x86_64.AppImage $OUTPUT
else
  ./appimagetool ./squashfs-root
fi

# Clean up
rm -rf AppDir linuxdeploy appimagetool es-temp.AppImage squashfs-root
